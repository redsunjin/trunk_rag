...
- 다음 구현은 `LOOP-069 reindex live adapter executor error sidecar draft`이며, 이 단계에서는 guarded executor failure를 `mutation_executor_error` sidecar와 promotion router failure route evidence로 노출한다.
