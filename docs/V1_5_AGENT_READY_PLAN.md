...
- 다음 구현은 `LOOP-048 reindex live adapter opt-in smoke harness draft`이며, 이 단계에서는 `LOOP-047`에서 고정한 explicit local-only binding seam을 default smoke와 분리된 별도 harness/command 관점으로 정리한다.
