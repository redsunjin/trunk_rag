...
- `config/actor_policy_manifest.json` + `services/actor_policy_service.py` 기준 actor policy resolver skeleton이 추가돼 runtime/middleware가 actor category별 read allowlist와 mutation candidate metadata를 읽는다.
