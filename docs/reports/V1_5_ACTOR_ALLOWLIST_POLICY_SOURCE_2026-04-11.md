...
- preview reference, intent summary, audit sink receipt를 묶는 apply envelope를 고정한다.
- preview reference 누락/불일치와 audit seed 누락을 다른 error code로 분리한다.
- 실제 write adapter 호출은 여전히 막아 두고 handshake 경계만 만든다.
