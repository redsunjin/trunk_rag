...
- `2026-04-07` closeout review에서는 no-reindex 제약 아래 채택 가능한 contextual retrieval 후보가 없다고 판단해 `LOOP-010`을 no-go로 닫고, 기본 경로는 계속 `mmr+light_hybrid+lexical_boost+coverage_rerank`로 유지하기로 했다.
- 다음 작업은 retrieval 후보 실험이 아니라, 본체 기본 runtime/reindex 경로에서 `all/eu/fr/ge/it/uk` sample-pack 번들이 자연스러운 기본처럼 드러나는 결합을 더 걷어내는 것이다.
