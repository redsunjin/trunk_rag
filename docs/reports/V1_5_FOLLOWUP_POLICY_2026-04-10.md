...
4. 다음 구현 후보는 `reindex executor activation seam draft`로 제한한다.
...
- 이후 구현은 actor policy source, mutation gate, preview/audit contract, preview seed/audit sink, mutation apply guard, execution go/no-go review, mutation executor interface draft, durable mutation audit backend skeleton 순서로 진행됐고 현재 다음 후보는 `reindex executor activation seam draft`다.
