...
- graph-lite JSONL 관계 스냅샷 로더/인메모리 검색/context append PoC 유지(`Quality` 단계 opt-in, `Balanced` 기본 경로 자동 적용 없음, 관계형/확산 답변 lead 보정, `/app` 답변의 hit/fallback/disabled 상태 노출 포함)
...
- graph-lite JSONL 관계 스냅샷 로더/인메모리 검색/context append PoC(`Quality` 단계 opt-in, no-hit/snapshot-missing 시 vector fallback, 관계형/확산 답변 lead 보정, `/app` 근거 요약/실행 상세의 hit/fallback/disabled 상태 노출 포함)
...
- `scripts/session_closeout.py`: 세션 종료 전 `roadmap_harness`, whitespace, git 상태, dirty worktree 정책 점검
