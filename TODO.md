...
| LOOP-044 | done | V1.5 reindex live adapter outline draft | `./.venv/bin/python -m pytest -q tests/test_mutation_executor_service.py tests/test_tool_audit_sink_service.py tests/test_agent_runtime_service.py tests/test_tool_middleware_service.py tests/test_smoke_agent_runtime.py` + `./.venv/bin/python scripts/roadmap_harness.py validate` |
| LOOP-045 | done | V1.5 reindex live adapter test plan draft | `./.venv/bin/python -m pytest -q tests/test_mutation_executor_service.py tests/test_tool_audit_sink_service.py tests/test_agent_runtime_service.py tests/test_tool_middleware_service.py tests/test_smoke_agent_runtime.py` + `./.venv/bin/python scripts/roadmap_harness.py validate` |
| LOOP-046 | done | V1.5 reindex live adapter success contract draft | `./.venv/bin/python -m pytest -q tests/test_mutation_executor_service.py tests/test_tool_audit_sink_service.py tests/test_agent_runtime_service.py tests/test_tool_middleware_service.py tests/test_smoke_agent_runtime.py` + `./.venv/bin/python scripts/roadmap_harness.py validate` |
| LOOP-047 | done | V1.5 reindex live adapter opt-in binding seam draft | `./.venv/bin/python -m pytest -q tests/test_mutation_executor_service.py tests/test_tool_audit_sink_service.py tests/test_agent_runtime_service.py tests/test_tool_middleware_service.py tests/test_smoke_agent_runtime.py` + `./.venv/bin/python scripts/roadmap_harness.py validate` |
| LOOP-048 | done | V1.5 reindex live adapter opt-in smoke harness draft | `./.venv/bin/python -m pytest -q tests/test_mutation_executor_service.py tests/test_tool_audit_sink_service.py tests/test_agent_runtime_service.py tests/test_tool_middleware_service.py tests/test_smoke_agent_runtime.py` + `./.venv/bin/python scripts/roadmap_harness.py validate` |
| LOOP-049 | done | V1.5 reindex live adapter executor injection protocol draft | `./.venv/bin/python -m pytest -q tests/test_mutation_executor_service.py tests/test_tool_audit_sink_service.py tests/test_agent_runtime_service.py tests/test_tool_middleware_service.py tests/test_smoke_agent_runtime.py` + `./.venv/bin/python scripts/roadmap_harness.py validate` |
| LOOP-050 | done | V1.5 reindex live adapter binding selection stub draft | `./.venv/bin/python -m pytest -q tests/test_mutation_executor_service.py tests/test_tool_audit_sink_service.py tests/test_agent_runtime_service.py tests/test_tool_middleware_service.py tests/test_smoke_agent_runtime.py` + `./.venv/bin/python scripts/roadmap_harness.py validate` |
| LOOP-051 | done | V1.5 reindex live adapter opt-in smoke command draft | `./.venv/bin/python -m pytest -q tests/test_mutation_executor_service.py tests/test_tool_audit_sink_service.py tests/test_agent_runtime_service.py tests/test_tool_middleware_service.py tests/test_smoke_agent_runtime.py` + `./.venv/bin/python scripts/roadmap_harness.py validate` |
...
- 기본 middleware chain은 `request_id`, `timeout_budget`, `tool_allowlist`, `unsafe_action_guard`, `audit_log` 순서로 동작한다.
- `ToolContext.timeout_seconds`를 추가해 runtime budget 정보를 middleware 입력과 registry adapter context에 연결했다.
- 쓰기 부작용 tool은 middleware의 unsafe action guard에서 먼저 차단하고, 기존 registry의 `ToolContext.allow_mutation` guard도 유지했다.
...
- closeout review에서는 기존 `/query`, `/health`, upload/admin API 계약을 건드리지 않고 WP2 skeleton이 완료됐다고 판단했고 다음 active loop는 `LOOP-019`로 승격했다.

## 완료 Loop (LOOP-019)

목표:
- `docs/V1_5_AGENT_READY_PLAN.md`의 WP3에 따라 한 요청에서 실행된 middleware/tool 단계와 실패 원인을 구조적으로 남기는 execution trace 계약을 고정한다.

범위:
- 포함: trace schema 초안, middleware trace와 tool result/error를 묶는 내부 계약, request_id/runtime budget/route reason seed 정리
- 제외: 사용자용 agent runtime, planner/worker, skill 자동 선택, MCP 통합, GraphRAG 재개

완료 기준:
- V1.5 tool/middleware 실행 결과의 trace schema가 문서와 테스트로 고정된다.
...
- closeout review에서는 기존 V1 API 계약을 건드리지 않고 WP3 trace contract가 완료됐다고 판단했고 다음 active loop는 `LOOP-020`으로 승격했다.

## 완료 Loop (LOOP-020)

목표:
- `docs/V1_5_AGENT_READY_PLAN.md`의 WP4에 따라 기존 `/query`를 대체하지 않는 내부 agent runtime entry draft를 시작한다.

범위:
- 포함: 단일 입력을 받아 allowlist/middleware/trace가 붙은 tool call 흐름을 테스트할 수 있는 내부 service entry
- 제외: 사용자 기본 `/query` 대체, planner/worker, skill 자동 선택, MCP 통합, GraphRAG 재개
...
범위:
- 포함: V1.5 신규 service 경계 검토, 문서/테스트/검증 결과 정리, 병합 전 리스크와 후속 작업 후보 확정
- 제외: 사용자 기본 `/query` 대체, 외부 MCP 통합, planner/worker 확장, GraphRAG 재개
...
범위:
- 포함: 내부 agent entry read-only 성공 경로와 write tool 차단 경로를 확인하는 smoke script 또는 테스트 보강
- 제외: public `/agent/*` endpoint, planner/worker, MCP 통합, GraphRAG 재개
...
범위:
- 포함: `redact_execution_trace()` 순수 함수, `internal/public/persisted` audience별 테스트, 민감 필드 제거/축약 규칙
- 제외: trace persistence 저장소, public `/agent/*` endpoint, MCP 통합, planner/worker 확장
...
- `read_query`, `read_admin`, `write_upload_review`, `write_index_maintenance`로 tool group을 나눴고 `internal_read_only`, `admin_read_only`, `admin_review_mutation`, `maintenance_mutation`, `unknown_read_only` actor category 초안을 정리했다.
...
- closeout review에서는 write tool 개방 전 필요한 actor policy source 경계가 충분히 정리됐다고 판단했고 다음 active loop는 `LOOP-028`로 승격한다.

## 완료 Loop (LOOP-028)

목표:
- `LOOP-027`에서 정리한 actor policy source 초안을 코드에서 읽을 수 있는 최소 resolver/manifest interface로 고정한다.

범위:
- 포함: actor category별 read allowlist, mutation candidate tool, policy flag resolver, runtime/middleware 연결
- 제외: 실제 admin auth 검증, mutation intent parser, dry-run preview payload, audit persistence 저장소

완료 기준:
- actor category 기준 policy lookup/resolver skeleton이 추가된다.
- agent runtime/middleware가 기본 allowlist를 상수 나열이 아니라 policy source 기준으로 읽는다.
...
진행 메모 (2026-04-11):
- `LOOP-027` closeout commit 후 actor policy source를 읽는 resolver/manifest skeleton으로 진입한다.
...
범위:
- 포함: admin actor 판별, mutation intent 신호, 차단 코드 분리, runtime/middleware/trace 반영
- 제외: dry-run preview payload, audit persistence 저장소, public `/agent/*` endpoint
...
## LOOP-032 closeout 메모 (2026-04-12)

- `docs/reports/V1_5_MUTATION_APPLY_DRAFT_2026-04-12.md`를 추가해 preview-confirmed mutation apply envelope schema, draft shape, error taxonomy를 고정했다.
- `services/tool_apply_service.py`를 추가해 `build_mutation_apply_envelope()`, `validate_mutation_apply_envelope()`, apply envelope schema version과 error code 상수를 구현했다.
- `services/agent_runtime_service.py`와 `services/tool_registry_service.py`는 `apply_envelope` 입력과 `apply_envelope_present` entry metadata를 추가했다.
- `services/tool_middleware_service.py`는 `PREVIEW_REQUIRED` 응답과 execution trace contract seed에 draft `apply_envelope`를 함께 노출하도록 변경했다.
- `tests/test_tool_apply_service.py`, `tests/test_tool_middleware_service.py`, `tests/test_agent_runtime_service.py`는 apply envelope schema와 preview/audit/intention error taxonomy를 검증하도록 갱신했다.
...
## LOOP-033 closeout 메모 (2026-04-12)

- `docs/reports/V1_5_MUTATION_APPLY_GUARD_2026-04-12.md`를 추가해 preview-confirmed apply guard flow, trace/error contract, 아직 execution을 열지 않는 이유를 고정했다.
- `services/tool_middleware_service.py`는 `mutation_apply_guard` middleware를 추가해 preview-confirmed envelope를 실제 guard 단계에서 검증하고 `PREVIEW_REFERENCE_MISMATCH`, `AUDIT_SINK_RECEIPT_REQUIRED`, `AUDIT_SINK_RECEIPT_INVALID`, `MUTATION_INTENT_SUMMARY_REQUIRED`, `MUTATION_APPLY_NOT_ENABLED`를 분리 차단하도록 변경했다.
- `services/tool_trace_service.py`는 middleware detail redaction allowlist에 `apply_envelope_present`를 추가해 guard 상태를 안전하게 남기도록 조정했다.
- `scripts/smoke_agent_runtime.py`와 `tests/test_smoke_agent_runtime.py`는 preview-confirmed apply path가 `mutation_apply_guard`에서 `MUTATION_APPLY_NOT_ENABLED`로 차단되는 smoke를 추가했다.
- closeout review에서는 mutation apply guard skeleton이 runtime/middleware/trace/smoke 경계에 고정됐다고 판단했고 다음 active loop는 `LOOP-034`, 다음 pending loop는 `LOOP-035`로 정리한다.

## 완료 Loop (LOOP-034)

목표:
- 실제 mutation execution을 열기 전에 필요한 go/no-go 기준, 사용자 결정 경계, backend/retention 전제조건을 문서로 고정한다.

범위:
- 포함: execution go/no-go review, decision matrix, backend/retention 선택지 정리, 다음 executor interface loop 정의
- 제외: 실제 write adapter 호출, persistence backend 구현, public `/agent/*` endpoint

완료 기준:
- 실제 write apply를 열기 전 필요한 사용자 결정 항목이 문서로 정리된다.
- mutation execution을 열지 않은 현재 이유와 해제 조건이 명확해진다.
- 후속 executor interface draft loop가 공식 큐에 고정된다.
...
- `docs/reports/V1_5_MUTATION_EXECUTION_GO_NO_GO_REVIEW_2026-04-17.md`를 추가해 실제 mutation execution의 `No-Go` 결론, decision matrix, activation precondition, `LOOP-035` 입력 계약을 고정했다.
- 현재 결론은 execution default `off`, public `/agent/*` 미개방 유지, durable audit backend/retention/operator ownership이 준비되기 전까지 `MUTATION_APPLY_NOT_ENABLED`를 유지한다는 것이다.
...
범위:
- 포함: mutation executor protocol, noop/default executor, tool adapter registry 경계, `reindex` 우선 adapter seam 설계
- 제외: 실제 write execution 활성화, durable backend 구현, upload review live adapter, public `/agent/*` endpoint

완료 기준:
- executor interface 초안이 문서와 테스트 기준으로 정리된다.
- 실제 write execution 없이도 adapter contract와 noop fallback 경계가 고정된다.
- 이후 activation 여부를 별도 결정으로 분리할 수 있다.
...
- `services/tool_audit_sink_service.py`는 `LocalFileAppendOnlyAuditSink`, env 기반 backend selection, 기본 root dir `chroma_db/mutation_audit/`, `sequence_state.json` 기반 stable sequence 발급을 제공하도록 확장했다.
- 기본 sink는 계속 `null_append_only`로 유지하고, `DOC_RAG_MUTATION_AUDIT_BACKEND=local_file`일 때만 durable backend가 선택되도록 제한했다.
- `tests/test_tool_audit_sink_service.py`는 local file sink receipt/entry/config selection 회귀를 추가했고, 타깃 회귀 `40 passed`로 backend skeleton을 확인했다.
- 다음 active loop는 `LOOP-037`이며, 이 단계에서는 `reindex` 전용 activation seam과 noop fallback 전환 조건을 정리한다.

## 완료 Loop (LOOP-037)

목표:
- `reindex` 전용 mutation executor activation seam을 문서/코드 기준으로 정리하고, noop fallback에서 live candidate로 넘어갈 조건을 한 군데로 모은다.

범위:
- 포함: reindex activation guard, operator explicit activation 조건, durable audit backend readiness 판정, selection seam
- 제외: 실제 live reindex execution, upload review executor, public `/agent/*` endpoint

완료 기준:
- `reindex` activation seam이 문서와 테스트 기준으로 정리된다.
- noop fallback에서 stub/live candidate로 넘어갈 조건이 분리된다.
...
진행 메모 (2026-04-18):
- `LOOP-036` closeout으로 durable append-only backend skeleton과 stable receipt metadata는 준비됐다.
- 이번 단계는 여전히 live execution이 아니라 `reindex` 전용 activation guard와 selection seam을 먼저 고정하는 것이다.
...
- `services/mutation_executor_service.py`는 `resolve_mutation_executor(request)`에서 `reindex` activation selection을 중앙화하고, `tool_registered`, `selection_state`, `selection_reason`, nested `activation` contract를 함께 남기도록 확장했다.
- `reindex`는 `DOC_RAG_AGENT_MUTATION_EXECUTION=1`과 durable `local_file_append_only` receipt(`sequence_id`, `storage_path`)가 함께 맞을 때만 candidate stub를 선택하고, 그렇지 않으면 registered noop fallback으로 남는다.
- `tests/test_mutation_executor_service.py`, `tests/test_tool_middleware_service.py`, `tests/test_agent_runtime_service.py`는 null sink fallback 경로와 local file candidate stub 경로를 각각 고정하도록 확장했다.
...
- 다음 active loop는 `LOOP-038`이며, upload review execution을 `reindex` activation seam과 섞지 않는 별도 boundary review를 진행한다. 다음 pending loop는 `LOOP-039 mutation audit retention ops draft`다.

## 완료 Loop (LOOP-038)

목표:
- upload review execution을 `reindex` activation seam과 분리된 별도 위험 경계로 정리하고, live candidate 범위 밖이라는 점을 문서/테스트 기준으로 고정한다.

범위:
- 포함: upload review executor boundary, rollback/audit precondition 구분, write tool별 단계적 activation 원칙 정리
- 제외: 실제 upload review live execution, `reindex` live execution, public `/agent/*` endpoint, prune job 자동화
...
- 이번 단계는 upload review execution을 계속 범위 밖으로 두되, 왜 `reindex`와 다른 rollback/audit 경계를 가져야 하는지 문서와 테스트 기준으로 분리하는 것이다.
...
- `services/mutation_executor_service.py`는 `approve_upload_request`, `reject_upload_request`를 generic unregistered noop가 아니라 `selection_state=boundary_noop`, `selection_reason=upload_review_scope_deferred`로 노출하고, nested `boundary` 계약에 rollback/audit/document binding precondition을 남기도록 확장했다.
- `approve_upload_request`는 `managed_doc_activation`, `reject_upload_request`는 `request_decision_only`로 분리해 managed markdown active 상태 변경과 decision-only write를 같은 live candidate로 취급하지 않도록 고정했다.
- `tests/test_mutation_executor_service.py`, `tests/test_tool_middleware_service.py`, `tests/test_agent_runtime_service.py`는 activation on + durable local audit ready 조건에서도 upload review가 계속 `boundary_noop`로 남는지 검증하도록 확장했다.
...
범위:
- 포함: `90일 rolling_window` retention, explicit prune ownership, operator-facing audit ops precondition
- 제외: 자동 prune job 구현, 실제 live execution 개방, public `/agent/*` endpoint, upload review live executor
...
목표:
- mutation activation blocked flow smoke 결과를 readiness evidence 형태로 정리하고, checklist 항목과 실행 증빙의 연결을 고정한다.

범위:
- 포함: smoke evidence 문서화, blocked flow 요약, readiness checklist와의 증빙 연결
- 제외: 실제 live execution 개방, upload review live executor, public `/agent/*` endpoint, prune 자동화 구현
...
closeout 메모 (2026-04-19):
- `scripts/smoke_agent_runtime.py`에 `v1.5.mutation_activation_smoke.v1` schema와 `apply_envelope`, `audit_sink`, `mutation_executor` summary를 추가해 blocked flow smoke를 readiness evidence로 직접 재사용할 수 있게 했다.
...
범위:
- 포함: checkpoint review 문서화, staged activation blocker 정리, upload review separate boundary 재확인
- 제외: 실제 live execution 개방, upload review live executor, public `/agent/*` endpoint, operator runbook 본문 작성
...
- `services/mutation_executor_service.py`의 `boundary.live_adapter_outline` contract에 같은 outline을 반영해 current noop fallback/candidate stub 경계와 future live adapter handoff 지점을 테스트 가능한 shape로 남겼다.
- `tests/test_mutation_executor_service.py`, `tests/test_tool_middleware_service.py`, `tests/test_agent_runtime_service.py`는 reindex boundary contract에 `live_adapter_outline`가 포함돼도 기존 blocked/candidate path가 유지되는지 검증하도록 갱신했다.
...
- 다음 active loop는 `LOOP-045`이며, 이번 outline contract를 기준으로 noop fallback/candidate stub/future live adapter smoke 범위를 test plan으로 정리한다.

## 완료 Loop (LOOP-045)

목표:
- `reindex` live adapter outline 뒤에 필요한 test plan과 smoke 범위를 문서화하고, current noop fallback/candidate stub contract와 future live adapter verification 경계를 고정한다.

범위:
- 포함: live adapter test plan 문서화, noop fallback/candidate stub/future live adapter 검증 축 정리, smoke/test ownership 메모
- 제외: 실제 live adapter 구현, actual live execution 개방, upload review adapter, public `/agent/*` endpoint
...
- 이번 단계는 actual execution을 열지 않고, outline contract가 요구하는 noop fallback/candidate stub/future live adapter smoke 범위를 test plan 형태로 정리하는 것이다.
...
- 공식 검증은 `./.venv/bin/python -m pytest -q tests/test_mutation_executor_service.py tests/test_tool_audit_sink_service.py tests/test_agent_runtime_service.py tests/test_tool_middleware_service.py tests/test_smoke_agent_runtime.py`, `./.venv/bin/python scripts/roadmap_harness.py validate`, `git diff --check` 통과로 마감한다.
- 다음 active loop는 `LOOP-046`이며, future live adapter success/failure result shape와 error taxonomy를 contract draft로 정리한다.

## 완료 Loop (LOOP-046)

목표:
- future `reindex` live adapter가 실제 enablement 전에도 따를 success/failure result contract와 error taxonomy 초안을 정리한다.

범위:
- 포함: success result shape 초안, failure taxonomy 초안, audit/rollback hint contract 메모
- 제외: 실제 live adapter 구현, actual live execution 개방, upload review success path, public `/agent/*` endpoint
...
- `services/mutation_executor_service.py`의 `boundary.live_adapter_outline`에 `success_contract`, `failure_taxonomy` metadata를 추가해 future opt-in smoke와 adapter-specific test가 기대할 result/error shape를 코드 계약에도 심었다.
- `tests/test_mutation_executor_service.py`, `tests/test_tool_middleware_service.py`, `tests/test_agent_runtime_service.py`는 expanded boundary contract가 추가돼도 current blocked/candidate path가 유지되는지 검증하도록 갱신했다.
- 공식 검증은 `./.venv/bin/python -m pytest -q tests/test_mutation_executor_service.py tests/test_tool_audit_sink_service.py tests/test_agent_runtime_service.py tests/test_tool_middleware_service.py tests/test_smoke_agent_runtime.py`, `./.venv/bin/python scripts/roadmap_harness.py validate`, `git diff --check` 통과로 마감한다.
- 다음 active loop는 `LOOP-047`이며, future live adapter를 default path에 섞지 않고 explicit local-only binding으로만 주입하는 selection seam을 정리한다.

## 완료 Loop (LOOP-047)

목표:
- future `reindex` live adapter를 default path를 바꾸지 않은 채 explicit local-only binding으로만 주입하는 selection seam 초안을 정리한다.

범위:
- 포함: opt-in binding source, selection precedence, default path guardrail, local-only activation 메모
- 제외: 실제 live adapter 구현, actual live execution 개방, upload review binding, public `/agent/*` endpoint
...
- `services/mutation_executor_service.py`의 `boundary.live_adapter_outline`에 `opt_in_binding` metadata를 추가해 binding source, selection precedence, required signals, guardrail을 코드 계약에도 반영했다.
- `tests/test_mutation_executor_service.py`, `tests/test_tool_middleware_service.py`, `tests/test_agent_runtime_service.py`는 opt-in binding seam metadata가 추가돼도 current blocked/candidate path가 유지되는지 검증하도록 갱신했다.
- 공식 검증은 `./.venv/bin/python -m pytest -q tests/test_mutation_executor_service.py tests/test_tool_audit_sink_service.py tests/test_agent_runtime_service.py tests/test_tool_middleware_service.py tests/test_smoke_agent_runtime.py`, `./.venv/bin/python scripts/roadmap_harness.py validate`, `git diff --check` 통과로 마감한다.
- 다음 active loop는 `LOOP-048`이며, explicit local-only binding seam을 default smoke와 분리된 별도 harness/command 관점으로 정리한다.

## 완료 Loop (LOOP-048)

목표:
- future `reindex` live adapter opt-in smoke를 default smoke와 분리된 별도 harness/command 관점으로 정리한다.

범위:
- 포함: opt-in smoke command 초안, local-only prerequisites, default smoke 분리 원칙, output evidence 메모
- 제외: 실제 live adapter 구현, actual live execution 개방, upload review smoke, public `/agent/*` endpoint
...
- `services/mutation_executor_service.py`의 `boundary.live_adapter_outline`에 `opt_in_smoke_harness` metadata를 추가해 prerequisites, expected evidence, isolation rule을 코드 계약에도 반영했다.
- `tests/test_mutation_executor_service.py`, `tests/test_tool_middleware_service.py`, `tests/test_agent_runtime_service.py`는 smoke harness metadata가 추가돼도 current blocked/candidate path가 유지되는지 검증하도록 갱신했다.
- 공식 검증은 `./.venv/bin/python -m pytest -q tests/test_mutation_executor_service.py tests/test_tool_audit_sink_service.py tests/test_agent_runtime_service.py tests/test_tool_middleware_service.py tests/test_smoke_agent_runtime.py`, `./.venv/bin/python scripts/roadmap_harness.py validate`, `git diff --check` 통과로 마감한다.
- `LOOP-049` closeout으로 explicit local-only binding carrier가 `AgentRuntimeRequest -> ToolContext -> MutationExecutionRequest` chain으로 고정됐고, request contract는 `executor_binding_present/kind/source/executor_name` signal을 남기게 됐다.

## 완료 Loop (LOOP-049)

목표:
- future `reindex` live adapter의 explicit local-only binding이 runtime/test harness에 어떻게 주입되는지 executor injection protocol 초안을 정리한다.

완료 메모 (2026-04-20):
- `docs/reports/V1_5_REINDEX_LIVE_ADAPTER_EXECUTOR_INJECTION_PROTOCOL_DRAFT_2026-04-20.md`를 추가해 local-only carrier chain, direct entrypoint, payload 차단 규칙을 문서화했다.
...
- executor contract request metadata에 binding presence/source/kind/executor name signal을 반영했다.

## 완료 Loop (LOOP-050)

목표:
- valid explicit live binding이 주입된 경우 `reindex_mutation_adapter_live` selection stub로 안전하게 전환되는 skeleton을 정리한다.

완료 메모 (2026-04-20):
- `docs/reports/V1_5_REINDEX_LIVE_ADAPTER_BINDING_SELECTION_STUB_DRAFT_2026-04-20.md`를 추가해 binding contract와 invalid fallback rule을 고정했다.
- `services/mutation_executor_service.py`에 `REINDEX_LIVE_ADAPTER_BINDING_KIND`, `_resolve_reindex_live_binding()`, `ReindexLiveMutationExecutorBindingStub`를 추가했다.
- activation guard를 통과하고 valid binding이 있을 때만 `selection_state=live_binding_stub`, `selection_reason=explicit_live_binding_requested`로 전환되게 했다.

## 완료 Loop (LOOP-051)

목표:
- opt-in live binding smoke를 default smoke와 분리된 concrete command surface로 고정한다.

완료 메모 (2026-04-20):
- `docs/reports/V1_5_REINDEX_LIVE_ADAPTER_OPT_IN_SMOKE_COMMAND_DRAFT_2026-04-20.md`를 추가했다.
- `scripts/smoke_agent_runtime.py`는 `--opt-in-live-binding`과 `DOC_RAG_MUTATION_SMOKE_LIVE_BINDING=1`을 지원하고, opt-in일 때만 apply request에 live binding을 주입한다.
- `tests/test_smoke_agent_runtime.py`는 default path의 `requested_live_binding=false`와 opt-in binding injection을 검증하도록 갱신됐다.

## 완료 Loop (LOOP-052)

목표:
- opt-in live binding smoke evidence를 실제 command output 기준으로 고정한다.
...
범위:
- 포함: success promotion rule 초안, blocked-success sidecar와 future apply success surface 매핑, smoke/runtime summary handoff 규칙 정리
- 제외: 실제 reindex side effect 개방, managed state write, upload review live execution, public `/agent/*` endpoint

완료 기준:
- future top-level success surface가 current blocked-success sidecar/smoke evidence와 충돌 없이 이어진다.
- success promotion 전환 지점이 문서와 테스트 seam 기준으로 정리된다.
...
- `boundary.live_adapter_outline.failure_taxonomy`는 같은 ordered metadata helper를 사용해 contract seed와 test seam이 갈라지지 않게 했다.
...
목표:
- `reindex` live adapter actual execution enablement를 열기 전 go/no-go 조건을 재검토한다.

범위:
- 포함: `mutation_apply_guard_execution_enabled` 전환 조건, operator activation + durable audit + explicit binding + success/failure taxonomy 준비도 평가, 실제 side effect enablement 전 blocker 정리
- 제외: actual reindex side effect 개방, public agent endpoint, upload review live execution, managed state rollback 구현

완료 기준:
- actual execution enablement가 가능한지 또는 추가 blocker가 남았는지 go/no-go로 판정된다.
...
진행 메모 (2026-04-21):
- `LOOP-056` closeout으로 success promotion과 failure taxonomy가 모두 contract/test seam 기준으로 고정됐다.
...
- durable audit receipt는 현재 blocked result metadata enrichment 과정에서 생성되므로, actual side effect 이전 pre-execution receipt handoff가 아직 보장되지 않는다.
...
목표:
- actual side effect를 열지 않고 `reindex` live adapter pre-execution audit/executor handoff seam을 고정한다.

범위:
- 포함: pre-execution durable audit receipt handoff, mutation executor router 위치, direct tool invocation 우회 차단 조건, top-level success/failure promotion handoff seam
- 제외: actual `index_service.reindex()` side effect 개방, public agent endpoint, upload review live execution, managed state rollback 구현

완료 기준:
- `mutation_apply_guard` 이후 actual write가 direct tool handler로 빠지지 않고 mutation executor handoff를 통과해야 한다는 계약이 문서/테스트 기준으로 고정된다.
...
진행 메모 (2026-04-21):
- `LOOP-057` closeout으로 actual execution enablement는 `No-Go`로 유지됐다.
...
범위:
- 포함: fake executor smoke contract, success/failure promotion smoke evidence, pre-execution handoff contract와 smoke summary 연결
- 제외: actual reindex side effect 개방, public agent endpoint, upload review live execution, managed state rollback 구현
...
목표:
- actual `index_service.reindex()` side effect를 열기 전 enablement checkpoint를 다시 검토한다.

범위:
- 포함: `LOOP-058` pre-execution handoff, `LOOP-059` fake smoke, `LOOP-060` router dry-run 이후 남은 blocker 재평가, Go/No-Go 판정
- 제외: actual reindex side effect 개방, public agent endpoint, upload review live execution, managed state rollback 구현
...
범위:
- 포함: mutation apply guard 이후 executor router handoff 위치, direct tool handler bypass, durable audit receipt precondition 연결, local-only explicit binding 유지
- 제외: actual reindex side effect 개방, public agent endpoint, upload review live execution, managed state rollback 구현
...
범위:
- 포함: direct tool handler bypass, durable audit receipt before side effect, explicit local-only binding, top-level promotion router, default smoke 유지 조건 재검토
- 제외: actual reindex side effect 개방, public agent endpoint, upload review live execution, managed state rollback 구현
...
목표:
- default smoke와 concrete skeleton smoke는 side effect 없이 유지하면서, guarded live executor stage를 명시적으로 선택하는 smoke command seam을 추가한다.

범위:
- 포함: smoke CLI/env stage selection, guarded stage request binding, fake smoke test proof, default/concrete path 회귀 방지
- 제외: 기본 smoke에서 actual side effect 실행, public route enablement, upload review live execution, real rollback drill
...
범위:
- 포함: guarded smoke command 실행, local-file audit receipt 확인, blocked top-level surface 유지 확인, runtime sidecar 요약 기록
- 제외: public route enablement, top-level success promotion gate 개방, upload review live execution, managed rollback drill
...
범위:
- 포함: guarded success evidence, executor error sidecar evidence, promotion router success/failure route, durable audit/post-executor evidence gap, rollback blocker 재점검
- 제외: top-level promotion gate 구현, public route enablement, upload review live execution, rollback drill 구현
...
범위:
- 포함: guarded promotion evidence, default blocked behavior, operator runbook 필요 여부, rollback drill blocker, next implementation scope
...
범위:
- 포함: runbook coverage, local-only promotion sufficiency, rollback drill blocker, default/public No-Go 유지 여부, 다음 loop 범위
- 제외: rollback drill 구현, default/public promotion 구현, upload review live execution
...
범위:
- 포함: harness 실행 output, pre/post/recovery vector evidence, audit linkage evidence, pass/fail 판정
- 제외: default/public promotion 구현, upload review live execution, rollback 자동화 확대
...
범위:
- 포함: merge/PR handoff, public blocker 구현, 다른 MVP/V1 항목 복귀 중 다음 진행 방향 결정
- 제외: 선택 전 default/public promotion 구현, upload review live execution 구현
...
목표:
- V1 웹 MVP에 맞는 최소 제품 정체성, SVG 브랜드 자산, 웹 UI 헤더 적용을 완료한다.

범위:
- 포함: Trunk RAG SVG mark/wordmark/favicon, `/assets` 정적 라우트, intro/app/admin 브랜드 헤더, README/SPEC 반영
- 제외: 대규모 UI 리디자인, 랜딩 페이지 전환, 제품명 변경, public blocker 구현, upload review live execution 구현
...
closeout 메모 (2026-04-29):
- `/query`는 `quality_mode=quality` 또는 `quality_stage=quality` 요청에서만 graph-lite snapshot을 읽고, hit 시 `[Graph-Lite Relations]` context 블록을 추가한다.
- Balanced/default 경로는 graph-lite를 로드하지 않으며, snapshot missing/no-hit/error는 기존 vector context로 fallback한다.
- API 응답은 `X-RAG-Graph-Lite` 헤더와 debug `meta.context.graph_lite`에 `disabled/not_run/hit/fallback`, fallback reason, relation count, context 추가 여부를 기록한다.
- `DOC_RAG_GRAPH_LITE_SNAPSHOT_DIR`로 기본 snapshot 위치를 바꿀 수 있게 했다.
- `qwen3.5:9b-nvfp4` quality graph-candidate 실측은 `3 cases`, `2 passed`, `pass_rate=0.6667`, `avg_weighted_score=0.8667`, `p95_latency_ms=15272.702`, `support_pass_rate=1.0`, `source_route_pass_rate=1.0`이었다.
...
범위:
- 포함: graph-candidate 답변 정책/프롬프트/평가 fixture 정합성 점검, `GQ-09` 실패 원인 축소, 기존 Quality graph-lite context 유지, 회귀 테스트와 live eval 재실행
- 제외: full Neo4j/GraphRAG 운영 도입, 기본 Balanced 경로 graph-lite 자동 적용, output token 예산 무조건 증가, UI 기본 Quality 모델 고정, 유료 API 호출

완료 기준:
- graph-candidate quality eval이 `3/3 pass`로 개선되거나, `GQ-09` 미통과 사유가 평가 fixture/문서 한계로 명확히 분리되어야 한다.
- generic/sample-pack 기본 경로와 Balanced 비적용 경로가 회귀하지 않아야 한다.
- latency가 기존 quality 실측 범위에서 크게 악화되지 않아야 한다.
- TODO/NEXT에 go/no-go 판단과 검증 결과가 남아야 한다.
...
closeout 메모 (2026-04-29):
- 관련 테스트 `tests/test_graph_lite_snapshot_builder.py tests/test_graph_lite_service.py tests/api/test_query_api.py -> 31 passed`.
- active-doc snapshot 생성 결과는 `source_docs=5`, `section_hits=21`, `entities=20`, `relations=48`이었다.
- 생성 snapshot 기준 graph-candidate benchmark는 `3/3 hit`, fallback `0`, `avg_latency_ms=0.179`, `avg_relation_count=8.0`이었다.
...
범위:
- 포함: graph-candidate quality eval/compare 재실행, active-doc snapshot 환경변수 적용, 결과 리포트/판단 문서화
- 제외: Balanced 기본 graph-lite 자동 적용, 모델 기본값 변경, 유료 API 호출, full Neo4j/GraphRAG 운영

완료 기준:
- graph-candidate Quality 평가 결과가 report로 남아야 한다.
- graph-lite context가 답변 품질과 support/source route에 미치는 영향이 go/no-go로 정리되어야 한다.
- 실패하거나 blocked이면 원인과 다음 보정 후보를 문서화해야 한다.
...
- 사용자가 다음 작업 진행을 지시했고, 다음 트랙을 `LOOP-129 Work status and verification spec`으로 확정했다.
- `LOOP-127`은 `main`/`origin/main`에 반영됐고, 별도 pending 항목 없이 명세서화 트랙으로 이어간다.

## 완료 Loop (LOOP-129)

목표:
- `LOOP-123`부터 `LOOP-127`까지 이어진 project-doc/user-doc 품질 게이트 작업을 하나의 추적 가능한 명세서로 고정한다.
- 무엇이 완료됐고, 어떤 명령으로 제대로 작업되는지 확인할 수 있는지 운영자/다음 세션 기준으로 정리한다.

범위:
- 포함: 완료 loop 요약, 산출물 경로, 검증 명령, live user-doc gate 실측 결과, 남은 gap/다음 후보 정리
- 제외: 새 기능 구현, default release gate 변경, desktop packaging blocked 해제, user-doc fixture 추가 확장

완료 기준:
- `docs/reports/WORK_STATUS_AND_VERIFICATION_SPEC_2026-05-06.md`가 현재 baseline, loop 산출물, 검증 matrix를 포함해야 한다.
- closeout 전에는 `TODO.md`와 `NEXT_SESSION_PLAN.md`가 `LOOP-129` active를 인식해야 하며, closeout 후에는 다음 대기 loop로 승격되어야 한다.
- roadmap harness가 ready여야 한다.
...
closeout 메모 (2026-05-06):
- `docs/reports/WORK_STATUS_AND_VERIFICATION_SPEC_2026-05-06.md`를 추가해 `LOOP-123~127` 산출물, 검증 matrix, live user-doc gate 실측 결과, 남은 후보 트랙을 한 문서로 고정했다.
- `2026-05-05 02:14 KST` live smoke에서 앱 서버 기동 후 `scripts/check_user_doc_quality_gate.py --json`이 `ready=true`, `UDQ-BC-01 1/1 passed`, `support_pass_rate=1.0`, `source_route_pass_rate=1.0`, `avg_weighted_score=1.0`으로 통과했음을 명세서에 반영했다.
...
- 사용자의 UI/UX 현대화 진행 지시를 다음 트랙 지정으로 보고 대기 상태를 닫았다.
- 방향 합의: `/app` 1차 개선, 기본 `Research Studio`, 고급 `AI Workspace` 성격의 오른쪽 `Advanced Rail`, 밝고 절제된 `Quiet Lab` 톤.
- 다음 작업은 `LOOP-143 Trunk RAG modern UI design spec`로 정했다.

## 완료 Loop (LOOP-143)

목표:
- `/app` UI/UX 현대화를 구현 전에 검토 가능한 디자인 스펙으로 고정한다.

범위:
- 포함: 기본 UX thesis, advanced rail, visual tone, `/app` 1차 범위, verification plan을 스펙 문서로 작성
- 제외: 실제 HTML/CSS/JS 구현, dark console theme, backend/API contract 변경, `/intro`/`/admin` 전체 redesign

완료 기준:
- `docs/superpowers/specs/2026-05-28-trunk-rag-modern-ui-design.md`가 확정 방향과 검증 계획을 포함한다.
- `TODO.md`와 `NEXT_SESSION_PLAN.md`가 다음 implementation plan 진입 상태를 가리킨다.
...
closeout 메모 (2026-05-28):
- `docs/superpowers/specs/2026-05-28-trunk-rag-modern-ui-design.md`를 추가했다.
...
범위:
- 포함: 사용자가 스펙을 승인하면 `writing-plans`로 `/app` UI 구현 계획 작성
- 제외: 승인 없는 직접 구현, `/intro`/`/admin` 전체 redesign, dark theme, backend/API 변경
