...
### 1) 실행/부트스트랩 단순화
- [ ] `run_doc_rag.bat` 개인 PC 경로 fallback 제거 또는 경고 명확화
- [ ] 브라우저 오픈 전 `/health` 준비 대기
- [ ] 서버 실행 실패 시 다음 행동(venv/python/포트) 안내
- [ ] 종료 스크립트/README 실행 흐름 동기화

### 2) 기본값 단일화
- [ ] 기본 provider/model/base_url 1세트 확정
- [ ] `.env.example`, UI 기본값, 런처 동작을 동일 기준으로 정렬
- [ ] `README.md`, `SPEC.md`의 실행/설정 설명 동기화

### 3) 사용자 화면 단순화
- [ ] 기본 모드에서 `provider/model/baseUrl/apiKey` 숨김 또는 접기
- [ ] 고급 설정 토글 제공
- [ ] 빈 인덱스 상태에서 `/reindex` 유도 메시지 제공
- [ ] 모델 미실행/연결 실패 시 가이드형 메시지 제공

### 4) 문서 등록 단순화
- [ ] `source_name` 자동 제안 또는 비워도 동작하도록 정리
- [ ] `country/doc_type` 기본값 자동 추론 강화
- [ ] 업로드 성공/보류/반려 후 다음 행동 안내

완료 기준:
- [ ] 새 사용자 기준 "실행 -> 상태 확인 -> 샘플 질의 -> 문서 업로드 요청"이 별도 설명 없이 가능하다.
- [ ] 기본 모드에서 사용자가 LLM 세부 설정을 직접 수정하지 않아도 된다.
