...
- 현재: `services/mutation_executor_service.py`는 preview-confirmed apply 이후에 붙는 executor activation/boundary seam을 제공하고, `reindex`는 operator activation request + durable local audit receipt가 함께 맞을 때 기본 `candidate_stub`, valid explicit binding이 추가되면 `live_binding_stub`를 선택하며, `boundary.live_adapter_outline`로 future live adapter 입력/출력/rollback awareness, injection protocol, opt-in smoke separation을 고정한다. upload review는 별도 `boundary_noop`로 유지한다
...
- 현재: `scripts/smoke_agent_runtime.py`는 `--opt-in-live-binding` 또는 `DOC_RAG_MUTATION_SMOKE_LIVE_BINDING=1`일 때 별도 live binding smoke path를 제공하고, activation/local-file backend 조합에서는 `reindex_mutation_adapter_live` `live_binding_stub` evidence를 남긴다
- 다음 우선순위: V1 회귀 게이트를 유지하면서 V1.5 `reindex` live adapter concrete executor skeleton을 정리하고, live scope는 여전히 `reindex` 단일 tool 후보로만 다룬다
