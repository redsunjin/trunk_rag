...
- 현재: `scripts/smoke_agent_runtime.py`는 `--opt-in-live-binding`, `--opt-in-live-binding-stage-concrete`, `DOC_RAG_MUTATION_SMOKE_LIVE_BINDING=1`, `DOC_RAG_MUTATION_SMOKE_LIVE_BINDING_STAGE`를 지원하고, concrete stage smoke에서는 `live_result_skeleton`과 `mutation_executor_result` evidence를 남긴다
- 다음 우선순위: V1 회귀 게이트를 유지하면서 V1.5 `reindex` live adapter top-level success promotion rule을 정리하고, live scope는 여전히 `reindex` 단일 tool 후보로만 다룬다
