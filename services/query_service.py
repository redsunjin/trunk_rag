...
Context에 [Graph-Lite Relations] 블록이 있으면 이를 관계 근거로 사용해 관계, 연쇄, 관계망, 역할을 질문 표현에 맞춰 직접 설명하세요.
관계형/확산 질문에서는 질문의 핵심 표현(연쇄, 확산, 관계망, 네트워크)을 답변에 직접 반영하고, 문서 근거가 있으면 지식 전파나 사상의 이동처럼 확산의 성격을 명확히 설명하세요.
질문이 연쇄나 흐름을 묻는다면 첫 문장부터 "이 연쇄는 ..."처럼 핵심 연결을 요약한 뒤 세부 근거를 설명하세요.
...
def extract_relation_chain_subject(question: str) -> str | None:
    match = re.search(r"^\s*(.+?)(?:이|가)?\s+어떤\s+(?:연쇄|흐름)", question.strip())
    if not match:
        return None
...
def build_relation_answer_lead(question: str, answer: str) -> str | None:
    if not any(keyword in question for keyword in ("연쇄", "흐름")):
        return None
    if any(keyword in answer for keyword in ("연쇄", "흐름")):
        return None

    subject = extract_relation_chain_subject(question)
    if not subject:
        return None

    if "확산" in question or "전파" in question or "계몽주의" in question:
        relation_tail = "지식 전파와 사상 확산으로 이어지는 흐름"
    else:
        relation_tail = "문서 근거의 관계 흐름"
    return f"이 연쇄는 {subject}{topic_particle(subject)} {relation_tail}입니다."
...
def postprocess_answer(question: str, answer: str, query_profile: str | None = None) -> str:
    ...
    relation_lead = build_relation_answer_lead(question, cleaned_answer)
    if relation_lead and relation_lead not in cleaned_answer:
        cleaned_answer = f"{relation_lead} {cleaned_answer}".strip()
