...
- 현재 active: `LOOP-144 Await implementation plan after modern UI design spec`
- design spec: `docs/superpowers/specs/2026-05-28-trunk-rag-modern-ui-design.md`
- confirmed UX: 기본 `/app`는 `Research Studio`, 고급 모드는 `AI Workspace` 성격의 오른쪽 `Advanced Rail`, visual tone은 밝고 절제된 `Quiet Lab`.
- first implementation scope: `/app`만 1차 개선한다. `/intro`와 `/admin`은 같은 시각 언어를 후속 loop에서 적용한다.
- explicit exclusions: dark `Signal Console`, full GraphRAG/Neo4j 재개, backend/API contract 변경, frontend framework 교체, desktop/browser companion redesign.
- next: 사용자가 스펙을 승인하면 `writing-plans`로 `/app` UI 구현 계획을 작성한다. 승인 전 직접 구현하지 않는다.
- verification: `./.venv/bin/python scripts/session_closeout.py --allow-dirty`, `./.venv/bin/python scripts/roadmap_harness.py validate`, `git diff --check`
...
- 완료 루프: `LOOP-134 Await next-track after user-doc fixture expansion`
...
- LOOP-134 closeout: 2026-05-18 사용자의 `진행` 지시를 다음 트랙 진행 신호로 보고 `LOOP-135 User-doc quality gate latest artifact freshness guard`를 선택했다.

## 2026-05-18 User-Doc Gate Freshness Guard Snapshot

- 완료 루프: `LOOP-135 User-doc quality gate latest artifact freshness guard`
- 완료 루프: `LOOP-136 Await next-track after user-doc gate freshness guard`
- 완료 루프: `LOOP-137 Local runtime smoke and live gate refresh`
- 현재 active: `LOOP-138 Await next-track after local runtime smoke`
- 작업 브랜치: `codex/loop-135-user-doc-gate-freshness`
- 목표: live eval을 다시 돌리지 않아도 persisted latest user-doc gate artifact가 fresh/ready인지 검사할 수 있게 한다.
- 코드 반영: `scripts/check_user_doc_quality_gate.py`에 `evaluate_latest_artifact_freshness()`, `build_latest_artifact_freshness_report()`, `--check-latest`, `--latest-artifact`, `--max-artifact-age-hours`를 추가했다.
- default freshness limit: `168` hours.
- stale diagnostic: `USER_DOC_GATE_ARTIFACT_STALE`.
- boundary: default release gate는 계속 `generic-baseline`이고, user-doc gate는 explicit `project_docs`/`user-doc-candidate` opt-in 경로로 유지한다.
- latest artifact 상태: 현재 저장본은 `2026-05-11T13:29:49+00:00` 생성본이므로 `2026-05-18` 기준 `--check-latest` smoke는 stale/blocked가 기대된다. 이 smoke는 live eval을 실행하지 않는다.
- 검증: target pytest `13 passed`; `py_compile scripts/check_user_doc_quality_gate.py` pass; `git diff --check` pass; `--check-latest --json` stale smoke `age_hours>168`, `USER_DOC_GATE_ARTIFACT_STALE`, exit `1` expected blocked; `roadmap_harness.py validate` ready.
- LOOP-136 closeout: 2026-05-21 사용자의 실제 구동 테스트 요청을 다음 트랙 지정으로 보고 대기 상태를 닫았다.
...
- 현재 active: `LOOP-138 Await next-track after local runtime smoke`
- 작업 브랜치: `codex/loop-135-user-doc-gate-freshness`
- 서버 실행: `./.venv/bin/python app_api.py`, base URL `http://127.0.0.1:8000`
- health: `release_web_status=ready`, `vectors=37`, `project_docs vectors=10`, embedding fingerprint `ready`, runtime profile `verified`, 기본 LLM `ollama/gemma4:e4b`.
- semantic smoke: `/semantic-search`가 `all` 컬렉션에서 `fr.md`, `uk.md` snippet을 반환했고 retrieval strategy는 `mmr+light_hybrid`였다.
- query smoke: 실제 `ollama/gemma4:e4b`, `timeout_seconds=60`, `debug=true`로 `/query` 성공 응답을 받았다. 질문의 파스퇴르 정보는 현재 샘플 문서에 직접 없어 answer는 not-found였지만 `support_level=supported`, citations `fr.md`, `uk.md`, `eu_summry.md`, invoke `ok`, `invoke_ms=14477.941`로 RAG 경로는 정상 확인됐다.
- UI smoke: Playwright 런타임으로 `/intro -> /app` 이동, `로컬 RAG 작업 공간`, `유럽 과학사 샘플 데모`, `default=all, vectors=37, llm=ollama` 상태 문구를 확인했다.
...
- ops baseline latest: `docs/reports/ops_baseline_gate_latest.json`, `docs/reports/OPS_BASELINE_GATE_LATEST.md` 생성. `/ops-baseline/latest`는 `ready=true`, `generic-baseline 3/3 passed`, `avg_weighted_score=0.96`, `p95_latency_ms=4843.711`을 반환했다.
- regression: `./.venv/bin/python -m pytest -q tests/api/test_system_api.py tests/api/test_query_api.py tests/e2e/test_web_flow_playwright.py -> 34 passed`.
- browser note: Browser plugin의 Node REPL 도구가 노출되지 않았고 `playwright-cli` wrapper는 `npx` 단계에서 응답이 없어 중단했다. 설치된 Playwright Python runtime으로 현재 서버 UI smoke를 대체했다.
- LOOP-138 목표: 자동 진행할 pending 항목이 없으므로 다음 트랙 결정을 대기한다.
...
- 같은 날짜 `services/query_service.py`는 기본 query profile을 `generic`으로 전환했고, 유럽사 전용 시스템 프롬프트와 `역할/비교/상징` 후처리 규칙은 `DOC_RAG_QUERY_PROFILE=sample_pack`일 때만 적용되도록 분리했다.
...
- 직접 Ollama 진단에서는 `gemma4:e4b`가 `avg_eval_tokens_per_second=40.088`, `assessment=borderline`이었다.
- 같은 날짜 후속 보강으로 `services/query_service.py` prompt에 영어 사고과정 서두 금지 문구를 추가하고, `Here's a thinking process...`/`Let me think...` 계열을 reasoning leakage 패턴으로 더 강하게 제거했다.
...
### A-Next22. V1.5 admin auth + mutation intent gate (완료: 2026-04-11)
...
closeout 메모 (2026-04-11):
- `services/tool_registry_service.py`의 `ToolContext`와 `services/agent_runtime_service.py`의 `AgentRuntimeRequest`에 `admin_code`, `mutation_intent` 입력을 추가해 runtime에서 mutation gate 신호를 분리했다.
...
- `services/tool_middleware_service.py`는 `mutation_policy_guard_middleware`를 추가해 mutation candidate write를 `ADMIN_AUTH_REQUIRED`/`ADMIN_AUTH_FAILED`/`MUTATION_INTENT_REQUIRED`/`PREVIEW_REQUIRED` 순서로 차단하고 trace/audit에 blocker를 남긴다.
- `services/agent_runtime_service.py`는 `admin_code_present`, `mutation_intent_present`를 entry metadata에 기록하고 read-only actor와 mutation actor의 allowlist 경계를 분리했다.
...
### A-Next25. V1.5 preview-confirmed mutation apply draft (완료: 2026-04-12)
...
- `services/tool_apply_service.py`를 추가해 apply envelope builder/validator와 error code 상수를 구현했다.
- `services/agent_runtime_service.py`, `services/tool_registry_service.py`는 `apply_envelope` 입력과 `apply_envelope_present` entry metadata를 추가했다.
- `services/tool_middleware_service.py`는 `PREVIEW_REQUIRED` 응답과 execution trace contract seed에 draft `apply_envelope`를 함께 남기도록 변경했다.
- `tests/test_tool_apply_service.py`, `tests/test_tool_middleware_service.py`, `tests/test_agent_runtime_service.py`는 apply envelope schema와 error taxonomy를 검증하도록 갱신했다.
...
### A-Next26. V1.5 mutation apply guard skeleton (완료: 2026-04-12)
...
진행 메모 (2026-04-12):
- `LOOP-032` closeout 이후 apply envelope draft를 실제 middleware guard helper에 연결하고 smoke까지 확장한다.

closeout 메모 (2026-04-12):
- `docs/reports/V1_5_MUTATION_APPLY_GUARD_2026-04-12.md`를 추가해 apply guard flow, trace/error contract, execution 미개방 원칙을 고정했다.
- `services/tool_middleware_service.py`는 `mutation_apply_guard`를 추가해 preview-confirmed apply envelope를 검증하고 `PREVIEW_REFERENCE_MISMATCH`, `AUDIT_SINK_RECEIPT_REQUIRED`, `AUDIT_SINK_RECEIPT_INVALID`, `MUTATION_INTENT_SUMMARY_REQUIRED`, `MUTATION_APPLY_NOT_ENABLED`를 분리 차단하도록 변경했다.
- `services/tool_trace_service.py`는 middleware detail redaction allowlist에 `apply_envelope_present`를 추가했다.
...
완료 기준:
- 실제 mutation apply를 열기 전 필요한 사용자 결정 항목이 문서로 정리된다.
- activation 해제 조건과 운영 blocker가 명확해진다.
- 다음 executor interface draft loop가 공식 큐에 고정된다.
...
완료 기준:
- mutation executor interface 초안이 문서와 테스트 기준으로 정리된다.
- 실제 write execution 없이도 adapter contract/noop fallback이 고정된다.
- 이후 실제 activation 여부를 별도 결정으로 분리할 수 있다.
...
### A-Next81. Brand PR merge/branch cleanup follow-up (완료: 2026-04-27)
...
### A-Next82. First-run recovery guide usability check (완료: 2026-04-28)
1. brand merge 이후 `/intro -> /app` 첫 실행 경로에서 사용자가 다음 행동과 복구 방법을 더 쉽게 확인하도록 한다.
2. `/health`의 `release_web_status`/`release_web_steps`를 intro/app에서 스캔 가능한 체크리스트로 보여 준다.
3. 명시 지시 없는 public blocker 구현, upload review live execution, GraphRAG 재개, 데스크톱 패키징 재착수는 수행하지 않는다.
...
진행 메모 (2026-04-28):
- `A-Next81`에서 브랜드 PR merge와 stale branch cleanup을 완료했다.
