...
3. 다만 현재 PoC는 여전히 Python 런타임, 의존성 설치, LLM 준비 상태에 기대므로 MVP 기본 경로로 넣지 않는다.
4. 제품화 후속의 다음 1순위는 문서 업로드/갱신 관리자 워크플로우 상세 설계다.
...
즉시 진행 대상 (다음 세션 1순위):
1. 문서 업로드/갱신 관리자 워크플로우 설계
2. GraphRAG 판단용 관계형 질문셋 수집
3. 데스크톱 패키징/배포 하드닝 여부 재검토

후속 대상 (P3):
1. 문서 업로드/갱신 관리자 워크플로우 설계(운영 절차 중심)
2. GraphRAG 필요성 검증 및 사이드카 PoC 여부 결정
3. 데스크톱 패키징/배포 하드닝 여부 재검토
